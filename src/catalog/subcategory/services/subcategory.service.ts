...
export class SubcategoryService {
  constructor(
    private subcategoryRepository: SubcategoryRepository //, private readonly categoryService: CategoryService
  ) {}
  ...
  async createSubcategory(subcategory: CreateSubcategoryDto): Promise<any> {
    ...
    // const category = await this.categoryService.findOne(subcategory.category_id)
    // if (!category) {
  ...
  async update(id: number, subcategory: UpdateSubcategoryDto): Promise<any> {
    ...
    // if (newSubcategory.category.id != subcategory.category_id) {
    //   const category = await this.categoryService.findOne(subcategory.category_id)
    //   if (!category) {
    //     throw new HttpException({message: 'The category does not exist!'}, HttpStatus.NOT_FOUND)
    //   }
